...
export class CartService {
  ...
  addItemByProductList(product: any, quantity: number) {
    const item = { ...product, quantity };
    this.addQuantitybyList(item, quantity);
  }

  addQuantitybyList(item: any, quantity: number) {
    console.log(item);
    ...
    if (this.model) {
      ...
      if (index >= 0) {
        ...
        this.model.items[index].quantity += quantity;
      } else {
        const items = [
          {
            item_id: item?.id,
  ...
  async getCart() {
    ...
    if (!data) {
      ...
      if (data?.value) {
        ...
        console.log('cart data = ',this.model);
        this.cart$.next(this.model);
      }
    }
  }
